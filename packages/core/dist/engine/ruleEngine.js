...
function evaluateRules(ctx) {
    ...
    const agent = db.prepare('SELECT rule_set_id FROM agents WHERE id = ?').get(ctx.agentId);
    const ruleSetIds = [];
    if (agent?.rule_set_id)
        ruleSetIds.push(agent.rule_set_id);
    // Also include default rule set
    const defaultSet = db.prepare('SELECT id FROM rule_sets WHERE is_default = 1').get();
    if (defaultSet && !ruleSetIds.includes(defaultSet.id))
        ruleSetIds.push(defaultSet.id);
    if (ruleSetIds.length === 0)
        return { allowed: true };
    const placeholders = ruleSetIds.map(() => '?').join(',');
    const rules = db
        .prepare(`SELECT id, type, is_enabled, action, priority, params
...
function checkRule(rule, params, ctx) {
    switch (rule.type) {
        case 'per_call_limit': {
        ...
        case 'daily_budget': {
            const limit = Number(params.limit ?? 0);
            if (limit > 0) {
                const spent = (0, budgetManager_js_1.getAgentDailySpend)(ctx.agentId);
        ...
        case 'monthly_budget': {
            const limit = Number(params.limit ?? 0);
            if (limit > 0) {
                const spent = (0, budgetManager_js_1.getAgentMonthSpend)(ctx.agentId);
                if (spent + ctx.estimatedAmount > limit) {
        ...
        case 'rate_limit': {
            ...
            if (maxCalls > 0) {
                const result = (0, rateLimiter_js_1.checkRateLimit)(ctx.agentId, ctx.targetService, {
        ...
        case 'method_restriction': {
            const allowed = params.allowed_methods ?? [];
